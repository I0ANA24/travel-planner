...
function SortableItem({ item }: { item: Location }) {
  ...
  return (
    <div
      ...
      className="p-4 border rounded-md flex justify-between items-center hover:shadow hover:cursor-grab transition-shadow"
    >
      <div>
        <h4 className="font-medium text-gray-800">{item.locationTitle}</h4>
        <p className="text-sm text-gray-500 truncate max-w-xs text-wrap">{`Latitude: ${item.lat}, Longitude: ${item.lng}`}</p>
      </div>
